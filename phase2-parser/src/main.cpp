...
/* Folds ownerAggregateKind into the printed Kind, so "Point::x" shows
   as "struct_member" and "Dog::legs" shows as "class_member" instead
   of both just showing the same generic "variable" -- the concrete
   distinction between "member of a struct" and "member of a class"
   that the plain SymKind enum doesn't carry on its own. Only variables
   and procedures get this treatment; parameters, labels, enum
   constants, and the tags themselves keep their existing plain label,
   since "struct_member" wouldn't add real information for those. */
...
static void printSymbolTable(std::ostream &out) {
    ...
    for (const auto &e : g_symbolTable) {
        ...
        snprintf(line, sizeof(line), "%-20s %-14s %-6d %-28s %-16s %-24s %-9d %-22s %s\n",
                 e.qualifiedName.c_str(), kindLabelFor(e).c_str(), e.declLine,
